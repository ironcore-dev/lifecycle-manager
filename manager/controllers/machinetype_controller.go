...
type MachineTypeReconciler struct {
	...
	BrokerClient lcmimachinetype.MachineTypeBrokerServiceClient
...
// +kubebuilder:rbac:groups=lifecycle.ironcore.dev,resources=machinetypes,verbs=get;list;watch;update;patch
...
func (r *MachineTypeReconciler) reconcile(ctx context.Context, obj *v1alpha1.MachineType) (ctrl.Result, error) {
	...
	if r.scanRequired(obj) {
		resp, err := r.BrokerClient.Scan(ctx, &lcmimachinetype.ScanRequest{
