...
// PackageInstallTaskReconciler reconciles PackageInstallTask objects.
type PackageInstallTaskReconciler struct {
	client.Client
...
func (r *PackageInstallTaskReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	obj := &v1alpha1.PackageInstallTask{}
...
func (r *PackageInstallTaskReconciler) reconcileRequired(
	ctx context.Context,
	obj *v1alpha1.PackageInstallTask,
) (ctrl.Result, error) {
...
func (r *PackageInstallTaskReconciler) reconcileNewTask(
	ctx context.Context,
	obj *v1alpha1.PackageInstallTask,
) (ctrl.Result, error) {
	log := logr.FromContextOrDiscard(ctx)
	...
	for _, mRef := range obj.Spec.Machines {
		...
		ok, err := r.validateMachineRef(ctx, key, obj.Spec.MachineTypeRef.Name)
		if err != nil {
			r.Recorder.Event(obj, corev1.EventTypeWarning, DanglingReference,
				fmt.Sprintf(danglingReferenceMessage, "Machine", mRef.Name))
			log.V(2).Info(fmt.Sprintf(danglingReferenceMessage, "Machine", mRef.Name))
			return ctrl.Result{}, err
		}
		if !ok {
			r.Recorder.Event(obj, corev1.EventTypeWarning, MachineTypeMismatch,
				fmt.Sprintf(machineTypeMismatchMessage, "Machine", mRef.Name))
			log.V(2).Info(fmt.Sprintf(machineTypeMismatchMessage, "Machine", mRef.Name))
			return ctrl.Result{}, nil
		}
...
func (r *PackageInstallTaskReconciler) validateMachineRef(
	ctx context.Context,
	...
	if err := r.Get(ctx, key, machine); err != nil {
		log.Error(err, "failed to get referenced machine object")
		return false, err
	}
	if machine.Spec.MachineTypeRef.Name != mType {
		return false, nil
	}
	return true, nil
}

func (r *PackageInstallTaskReconciler) processPackages(
	ctx context.Context,
	obj *v1alpha1.PackageInstallTask,
	mRef string,
) (uint32, error) {
	...
	for _, pRef := range obj.Spec.Packages {
		...
		validPackageRefs += 1
		updateJob := &v1alpha1.PackageInstallJob{
			...
			Spec: v1alpha1.PackageInstallJobSpec{
				MachineRef:         corev1.LocalObjectReference{Name: mRef},
...
func (r *PackageInstallTaskReconciler) validateFirmwarePackageRef(
	ctx context.Context,
	key types.NamespacedName,
	mType string,
) (bool, error) {
...
func (r *PackageInstallTaskReconciler) reconcileExistingTask(
	ctx context.Context,
	obj *v1alpha1.PackageInstallTask,
) (ctrl.Result, error) {
	log := logr.FromContextOrDiscard(ctx)
	jobs := &v1alpha1.PackageInstallJobList{}
...
func (r *PackageInstallTaskReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.PackageInstallTask{}).
		Owns(&v1alpha1.PackageInstallJob{}, builder.WithPredicates(r.machineUpdateJobPredicates())).
...
func (r *PackageInstallTaskReconciler) machineUpdateJobPredicates() predicate.Predicate {
	return predicate.Funcs{
		UpdateFunc: r.enqueueByOwner,
	}
}

func (r *PackageInstallTaskReconciler) enqueueByOwner(e event.UpdateEvent) bool {
	objOld, okOld := e.ObjectOld.(*v1alpha1.PackageInstallJob)
	objNew, okNew := e.ObjectNew.(*v1alpha1.PackageInstallJob)
	if !okOld || !okNew {
		return false
	}
...
func ownerRefFromTask(obj *v1alpha1.PackageInstallTask) metav1.OwnerReference {
